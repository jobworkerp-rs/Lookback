...
The same staging step also satisfies Tauri's `plugins/*.so`, `plugins/*.so.*`,
and `*.dylib` resource globs. Those globs are resolved from `src-tauri`, so the
bundle staging destination is `agent-app/src-tauri/plugins/`. The `plugins/*.so.*`
glob is required for versioned Linux runtime libraries such as `libcudart.so.12`
and SONAME symlinks. If `LOOKBACK_PLUGINS_SRC` is set, it is searched
