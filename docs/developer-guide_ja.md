...
2. 失敗した場合は画面のエラー、または `<data-root>/log/lookback.log` を確認します。接続失敗は
   `jobworkerp connection failed (<url>)` または `memories connection failed (<url>)` の形で記録されます。
3. より詳細なログが必要な場合は `LOOKBACK_RUST_LOG=debug` を付けて起動し、同じ操作を再実行します。
4. 接続テストが成功しても一覧が空の場合、接続自体は成功しています。リモート memories 側に対象データが
