...
if [[ "${LIBEXT}" == "so" ]]; then
  versioned_placeholder="${PLUGINS_DIR}/libplaceholder_ci.so.0"
  [[ -e "${versioned_placeholder}" ]] || : > "${versioned_placeholder}"
fi

echo "staged CI placeholders for ${TRIPLE} (bin/*, plugins/*.${LIBEXT}, versioned Linux .so runtime glob, dict/)" >&2
